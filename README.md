...
- **Compliance Report PDF** — branded one-page report with dataset summary, compliance scores, PHI detection results, and transformation descriptions; compliance scores and PHI confidence levels are conditionally formatted for quick visual scanning
...
Use any of the CSV files in `data/raw/` to test the toolkit without needing your own data.
...
See the `example_reports/` folder for sample PDF outputs across all three modes, with conditional formatting applied to compliance scores and PHI confidence levels:
- Audit Mode (with logo)
- Generalize Mode (without logo)
- Full De-Identification Mode (with logo)

> **Note:** Example reports were generated progressively as the toolkit was built; conditional formatting was added in a later update. All newly generated reports include conditional formatting regardless of mode.
...
See `assets/` for screenshots of the RBAC verification in action.
...
```
hipaa-deidentification-pipeline/
│
├── FakePatientData.py
├── HIPAA_De-Identification_Pipeline.sql
├── hipaa_deidentification_toolkit.py
├── requirements.txt
├── LICENSE
├── README.md
│
...
│   │   └── insurance.csv
│   └── deidentified/
│       └── (de-identified CSVs generated by the toolkit)
│
├── example_reports/
│   ├── EXAMPLE Audit Mode Compliance Report w Logo.pdf
│   ├── EXAMPLE Generalize Mode Compliance Report without Logo.pdf
│   └── EXAMPLE Full De-Identification Compliance Report w Logo and Conditional Formatting.pdf
│
└── assets/
    ├── rbac_role_set.png
    └── rbac_permission_error.png
```
...
| `data/raw/` | Generated fake patient, medical, and insurance CSVs |
| `data/deidentified/` | Output folder for de-identified CSVs generated by the toolkit |
| `example_reports/` | Sample compliance report PDFs for all three modes |
| `assets/` | Screenshots of RBAC verification — role set and permission denied error |
...
```
This outputs `patients.csv`, `medical_records.csv`, and `insurance.csv` to `data/raw/`.
